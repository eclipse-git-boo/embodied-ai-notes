...
<figure class="paper-figure">
  <img src="{{ '/assets/paper-figures/samples/openvla-fig2-architecture-crop.png' | relative_url }}" alt="OpenVLA 模型的动作 token 化与反 token 化框图">
  <figcaption>论文 Fig. 2（裁切）：连续动作逐维量化为词表 token，再由 action de-tokenizer 恢复。量化/反量化规则必须和 checkpoint 绑定。</figcaption>
</figure>
...
<figure class="paper-figure">
  <img src="{{ '/assets/paper-figures/samples/openvla-fig2-architecture-crop.png' | relative_url }}" alt="OpenVLA 动作 token 模型框图">
  <figcaption>论文 Fig. 2（裁切复用）：该接口图解释了实验结果为何不能脱离动作空间来比较——基线必须使用同一动作语义、量化规则与评测协议。</figcaption>
</figure>
